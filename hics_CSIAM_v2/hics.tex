...
The SMP is a new concept that might be not a minimizer but indicates the distance between
...
to discretize subproblem through simplex and its rotaion in which the discretization
points linearly increase as the spatial dimension. We apply the HiCS to several
benchmarks.  Numerical results demonstrate that the HiCS can efficiently obtain a
...
methods to further find a minimizer. Alternatively, we can also decrease the search
...
criterion is met.  
The HiCS finds a neighbourhood that contains extreme value points
...
\Note{
In this paper, we prove that the HiCS converges to a SMP in finite steps. 
}
