...
high-dimensional benchmarks.
	...
	\keywords{
Hill-climbing method with a stick, Suspected extreme point, Finite-step convergence, 
...
\textcolor{blue}{ 
The power increasing discretization points as the dimension $d$
...
In the Algorithm\,\ref{alg:AHiCS}, we can change the search radius $\rho$ by
setting $\rho=\eta\rho$ ($\eta<1$), if a SMP
is found for a fixed $\rho$. A small $\rho$ also shortens 
the distance between the SMP and a minimizer. As $\rho$ go to $0$, the SMP might
becomes a minimizer.
Certainly, the search surface can be expanded by setting regulator
...
%    \centerline{(b) }
	\end{minipage}
	  ...
	  1000]^{1000}$, $\rho=2.0$ and regulator
...
Determining a small area that contains minimizers is a prerequisite for the
success of optimization methods.  
Inspired by the hill-climbing behavior of the blind, we 
proposed the HiCS to find a satisfactory area with a search radius. 
More significantly, we gave a new concept of the SMP to precisely present the HiCS. 
By combining with the SMP, the HiCS can be 
The SMP can be used to describe 

We also give 


new derivative-free method to unconstrained
...
We proposed a new simplex discretization method to save computational amount to
address high-dimensional problems in this work. Using the
